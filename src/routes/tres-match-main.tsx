...
import { LandPlot, ListOrdered, PlayIcon, Volleyball } from 'lucide-react';
import { NavLink, useNavigate } from 'react-router';

const NUMBER_OF_PLAYERS_OPTIONS = [3, 4, 5];

export const TresMatchMain = () => {
  const { numberOfPlayers, onSetNumberOfPlayers, activeGame } = useTresLogic();
  ...
  return (
    <div className='mt-10'>
      ...
      {!activeGame ? (
        ...
        <div className='p-4 mt-5 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm'>
          <div className='flex items-center justify-center flex-col text-sm font-bold flex-wrap'>
            ...
            <NavLink
              to='/frame-scoring'
              className='font-bold underline w-full bg-primary rounded-lg p-2 text-background-dark mt-4 text-center'>
              Continue
            </NavLink>
          </div>
        </div>
